...
def build_fo76_images(diffuse_path: Path, normal_path: Path, spec_path: Path,
    ...
    """Returns (images, has_alpha, target_size). images is a dict that may
    contain keys 'diffuse', 'normal', 'reflection', 'lightmap' -> RGBA PIL Images."""

    images = {}
    img_d_raw = Image.open(diffuse_path) if diffuse_path and diffuse_path.exists() else None
    target_size = img_d_raw.size if img_d_raw else (2048, 2048)
    ...
    if original_diffuse_img is not None:
...
class TextureEditState:
    ...
    def apply(self, base_img: "Image.Image") -> "Image.Image":
        """Composites the paint layers + adjustments onto base_img. base_img may be
        a different resolution than the stored masks/values (e.g. the live preview
        panel works at a reduced resolution while export/editing use full res) -
        in that case a resized COPY is used for compositing only; the stored
        full-resolution paint data is never touched, so nothing is lost."""
...
def process_texture_set_pbr(diffuse_path: Path, normal_path: Path, spec_path: Path,
    ...
    if "diffuse" in images:
        ...
        save_dds(img_d_out, output_dir / f"{base_name}_d.dds", bc_fmt, log_func=log_func)

    if "normal" in images:
        save_dds(images["normal"].convert("RGB"), output_dir / f"{base_name}_n.dds", "BC5_SNORM",
    ...
    if "reflection" in images:
        ...
        save_dds(images["reflection"].convert("RGB"), output_dir / f"{base_name}_r.dds", r_fmt, log_func=log_func)

    if "lightmap" in images:
        save_dds(images["lightmap"].convert("RGB"), output_dir / f"{base_name}_l.dds", "BC1_UNORM", log_func=log_func)
...
class FO76PBRStudioGUI:
    def __init__(self, root):
        ...
        self.default_brush_color = (180, 180, 185)
    ...
    def create_widgets(self):
        ...
        right_panel = ttk.LabelFrame(main_container, text=" Texture Set Preview & Editing ", padding="10")
        ...
        # 4. TEXTURE SET PREVIEW & EDITING PANEL
        ...
        for idx, kind in enumerate(self.preview_kinds):
            r, c = divmod(idx, 2)
            tile = ttk.Frame(grid_frame, padding=4, relief=tk.GROOVE, borderwidth=1)
    ...
    def _flush_dirty(self):
        ...
        self._dirty_maps = set()
        if maps:
            self.update_texture_previews(maps)
    ...
    def set_default_brush_color(self, rgb_tuple):
        self.default_brush_color = rgb_tuple
        self.brush_color_swatch.config(bg=self._hex(rgb_tuple))

    def pick_default_brush_color(self):
        color_code = colorchooser.askcolor(title="Choose Default Brush Color")
    ...
    def run_batch(self, in_dir, out_dir, params, use_bc7_diffuse, use_bc7_reflection):
